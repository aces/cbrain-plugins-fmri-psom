...
class CbrainTask::NiakPipelineFmriPreprocess < PortalTask
  ...
  def after_form #:nodoc:
    ...
    if FmriStudy.where(id: params[:interface_usefile_ids]).all.to_a.any? { |u| u.name == params[:output_name] && u.data_provider_id == ( self.results_data_provider_id || u.data_provider_id) }
  ...
  def final_task_list #:nodoc:
    ...
    task_list = []
    file_ids.each do |id|
      mj = self.dup # not .clone, as of Rails 3.1.10
